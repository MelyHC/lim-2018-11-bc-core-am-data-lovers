...
const Pokemon = ({ pokemons, pokeActual }) => {
  ...
  return (
    <Fragment>
      <Link to="/lim-2018-11-bc-core-am-data-lovers" className="fas fa-angle-left arrow position-fixed text-secondary m-2 ml-md-4"></Link>
      ...
      <section className="row m-0">
        ...
        <div className="col-md-6 text-center text-md-left">
          ...
          <article className="mb-2">
            <h5>Tipo:</h5>
            {pokemon.type.map(strType => <span className="text-size" key={strType}>- {strType} </span>)}
          </article>
          <article className="mb-2">
          ...
          <article className="mb-1">
            <h5>Debilidades:</h5>
            {pokemon.weaknesses.map(strWeakness => <span className="text-size" key={strWeakness}>- {strWeakness} </span>)}
          </article>
        </div>
      </section>
      <hr className="w-75 bg-light-grey" />
